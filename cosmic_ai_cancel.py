...
def lambda_handler(event, context):
    ...
    if int(players) == 0:
        ...
        if int(players) < 8:
            if paiva == "Thursday":
                send_message_to_slack("Huomisen vuoro peruttu, mutta voitte kokeilla toki josko saatte viereisen kentän kanssa pelit.")
            else:
                send_message_to_slack("Tämän päivän vuoro peruttu")
