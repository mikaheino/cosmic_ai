# Cosmic AI
# pääkonttorin_saba -Slack integration
...
if int(players) == 0:
        print("Mitä ihmettä? Ei yhtään pelaajia seuraavaan säbään? \nTäältä pääsee ilmoittautumaan jos linkki on hukassa.")
else:
    if int(players) > 8:
            quote = [f"Vuoristoneuvosto Tuurakin olisi ylpeä! {players} on ilmoittautunut seuravaan säbään", f"Ei tarvitse häpeillä yhtään tai huudella nimiä, koska {players} nimeä jo listalla säbään."]
            print("<!here> " + random.choice(quote) + "\n\nMuistutuksena vielä kaikille vuorojen ilmottautumisdeadlinet. Tiistain vuorolle se on tiistaina klo 12:00 ja perjantain vuorolle se on edeltävänä torstaina klo 21:00. Jos vuorolle on deadlineen mennessä ilmoittautunut vähintään 8 pelaajaa, niin vuoro pidetään. Jos ilmoittautujia on 7 tai vähemmän, vuoro perutaan.\n\nTäältä pääsee ilmoittautumaan jos linkki on hukassa.")
    else:
